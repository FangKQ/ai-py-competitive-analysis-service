...
在技术实现层面，系统采用Python FastAPI作为后端框架，React + TypeScript构建前端交互界面，通过SSE（Server-Sent Events）实现Agent执行过程的实时流式展示。核心的Agent Loop实现了ReAct（Reasoning and Acting）循环机制，并创新性地引入了Ratchet（棘轮）机制防止Agent在推理过程中出现回退和循环错误。Governance治理层实现了Token预算控制、权限管理和完整的审计日志，确保系统在生产环境中的安全可控。
...
\node[comp,fill=bytedance!5] at (-4.5,0) {React前端};
...
前端采用React + TypeScript构建，提供以下核心交互能力：
...
\toprule
\textbf{技术} & \textbf{版本} & \textbf{选择依据} \\
\midrule
Next.js & 14 & React框架标准选择，支持SSR/SSG/App Router \\
React & 19 & 最新版，支持Server Components和并发特性 \\
TypeScript & 5.0+ & 类型安全，配合后端Pydantic实现端到端类型一致 \\
Tailwind CSS & v4 & 原子化CSS，快速构建一致UI \\
React Flow & latest & DAG可视化事实标准，自定义节点和动画 \\
Recharts & latest & React生态图表库，竞品数据可视化 \\
\bottomrule
\end{tabularx}
\end{table}
...
DAG可视化是系统可观测性的核心展示组件。使用React Flow实现，节点状态通过颜色和动画实时指示：

\begin{itemize}
\item \textbf{灰色（Pending）}：等待前序节点完成
\item \textbf{蓝色闪烁（Running）}：Agent正在执行中
\item \textbf{绿色（Completed）}：执行完成，可展开查看结果
\item \textbf{红色（Failed）}：执行失败，可查看错误详情
\end{itemize}

每个Agent节点采用不同的图标和主题色以增强视觉辨识度：Orchestrator用紫色指挥棒图标，Collector用青色搜索图标，Analyst用绿色图表图标，Writer用橙色笔图标，Reviewer用红色放大镜图标，Citation用紫色链接图标。

节点间的连线采用贝塞尔曲线，数据流动方向从上到下，并行节点水平排列。当数据在节点间传递时，连线会出现流动动画效果。

\section{Agent Trace面板设计}

Trace面板采用垂直时间线设计，展示每个Agent的完整决策历程。设计参考了OpenTelemetry的分布式追踪可视化范式：

\begin{enumerate}
\item 每条日志条目显示：时间戳、Agent角色图标、执行动作描述
\item 展开后显示：完整推理过程（reasoning）、工具调用详情、输入/输出Token数量、执行耗时
\item 支持按Agent角色过滤和时间范围筛选
\item 工具调用的输入参数和返回结果以可折叠代码块展示
\item 底部汇总栏显示总Token消耗、总耗时和Agent调用次数
\end{enumerate}
...
\begin{itemize}
\item \textbf{执行摘要卡片}——突出显示3-5条关键发现，使用醒目的数字指标
\item \textbf{竞品概览网格}——每个竞品一张信息卡，包含logo、简介、核心数据
\item \textbf{功能对比表格}——横向对比矩阵，使用颜色编码表示优势(绿)/中等(黄)/劣势(红)
\item \textbf{SWOT四象限图}——视觉化展示Strengths/Weaknesses/Opportunities/Threats
\item \textbf{引用高亮}——正文中[1][2]引用标记可点击，悬停显示来源URL和原文片段
\item \textbf{报告导出}——支持一键导出为Markdown和PDF格式
\end{itemize}

\section{SSE实时状态更新机制}

前端通过SSE（Server-Sent Events）接收后端推送的实时事件。这是一个比WebSocket更轻量的单向推送方案，非常适合Agent执行状态的实时展示场景：

\begin{enumerate}
\item 创建分析任务后，前端建立到\texttt{/api/tasks/\{id\}/events}的SSE连接
\item 接收到\texttt{node\_started}事件时，更新DAG节点为Running状态（蓝色闪烁）
\item 接收到\texttt{node\_completed}事件时，更新为Completed状态（绿色），Trace面板追加日志
\item 接收到\texttt{dag\_completed}事件时，自动加载完整报告并渲染
\item 连接异常时实现指数退避重连（初始1秒，最大30秒）
\end{enumerate}
