...
\begin{tabular}{rl}
\textbf{参赛者:} & 戴尚好 \\[0.3cm]
\textbf{学校:} & 中国科学技术大学 \\[0.3cm]
\textbf{联系邮箱:} & bcefghj@163.com \\[0.3cm]
\textbf{GitHub:} & github.com/bcefghj \\[0.3cm]
\textbf{提交日期:} & 2026年5月16日 \\[0.3cm]
\textbf{文档版本:} & v2.0 \\
\end{tabular}
}
...
\item \textbf{Live Demo}——三个场景卡片+「进入完整Demo体验页」按钮，链接到独立的\texttt{demo.html}
...
\section{双页面架构与Demo交互设计}

系统采用\textbf{介绍页+独立Demo体验页}的双页面架构（类似优秀Hackathon项目的Landing Page→Interactive Demo模式）：

\begin{itemize}
\item \textbf{介绍页}（\texttt{index.html}）——12章节叙事弧，从痛点到解决方案，包含Agent协作模拟器
\item \textbf{Demo体验页}（\texttt{demo.html}）——独立的交互式Demo，双面板布局：
  \begin{itemize}
  \item 左面板（40\%）：Agent协作时间线，逐步动画展示每个Agent的推理过程、工具调用和Token消耗
  \item 右面板（60\%）：完整Markdown报告渲染区，支持表格、有序列表、引用块等完整语法
  \item 底部：Agent决策追踪Trace折叠面板，展示每步的详细推理链
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
\item \textbf{预跑场景（零等待）}：三个场景（AI对话助手/短视频平台/AI编程工具）均有预跑的高质量分析报告，点击Tab即可秒开完整报告，无需等待API调用
\item \textbf{自定义分析（实时）}：评委可输入任意竞品分析需求，系统调用MiniMax M2.7大模型，由6个Agent实时协作生成报告。通过SSE实时推送执行事件，API不可用时自动Fallback到预设Demo数据
\end{enumerate}
...
\item \texttt{/demo.html} — 独立Demo体验页（双面板Agent时间线+报告渲染）
